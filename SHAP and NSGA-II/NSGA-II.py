...
def evalWithConstraints(individual, model):
    ...
    if individual[2] <= individual[0] /2:
        return 1000, 1000 # width of steel plate and timber panel
    if min(F1,F2,F3) <= 180000:
        return 1000,  1000# bearing capacity
    ...
    return -fire_resistance_time, weight
...
# Create type
creator.create("FitnessMin", base.Fitness, weights=(-1.0,-1.0))  # Fitness Min, minmize
...
def main():
    # generate population
    population = toolbox.population(n=1500)
    ...
    algorithms.eaMuPlusLambda(
        population, toolbox, mu=1500, lambda_=3000, cxpb=0.6, mutpb=0.1, ngen=50,
    ...
    plt.scatter(weights, fire_resistances, color='b', s=10, label='Pareto Front')
    plt.xlabel("Structure Weight")
    plt.ylabel("Fire Resistance (min)")
    plt.title("Pareto Front for Fire Resistance vs. Structure Weight")
    plt.legend()
    plt.grid()
    plt.show()
    ...
    columns = ["Wood Width", "Wood Thickness", "Iron Width", "Iron Thickness", "Edge Distance", "Bolt Diameter",
               "Bolt Number", "Bolt Type", "Load Ratio", "Weight", "Fire Resistance Time"]
